...
class TC720():
    ...
    def float_to_hex(self, value, scale=100):
        """
        Converts a float (can be negative) to a 4-character hexadecimal string.
        Handles two's complement for negatives.
        """
        integer = int(round(value * scale))  # scale and convert to integer

        if abs(integer) > 32768:
            raise ValueError(
                'Can not encode positive or negative numbers larger than 32768 in length 4 hexadecimal.'
            )
        ...
        return '{:0>4}'.format(hex(integer)[2:])
    ...
    def set_soak_temp(self, location, temperature):
        ...
        Input:
            location (int): 1-8
            temperature (float): Degrees Celsius, can be negative
        """
        if type(location) != int or not (1 <= location <= 8):
            raise ValueError(
                f'Invalid location: "{location}". Must be an integer in range 1-8.'
            )
        
        self.check_mode(1)
        location_code = 'a' + str(location-1)

        # Use float_to_hex directly
        hex_temp = self.float_to_hex(temperature)

        # Set soak temperature
        self.send_message(self.message_builder(location_code, hex_temp), write=True)
